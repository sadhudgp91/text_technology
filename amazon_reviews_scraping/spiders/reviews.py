...
class AmazonReviewsSpider(scrapy.Spider):
    ...
    def __init__(self, product_id=None, *args, **kwargs):
        ...
        self.start_urls = ['https://www.amazon.com/product-reviews/' +product_id +'/']
    ...
    def extract_reviews(self, response):
        for review in response.css('div[data-hook=review]'):
            yield {
                ...
                'stars': review.css('span.a-icon-alt::text').extract_first(), 
                'title': review.css('a.review-title::text').extract_first(),
                'author_profile_url': review.css('span.a-profile-name::text').extract_first(),
                'author_name': review.css('a[data-hook="genome-widget"]::text').extract_first(),
